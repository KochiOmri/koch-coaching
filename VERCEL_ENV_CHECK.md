# ⚠️ VERCEL ENVIRONMENT VARIABLES - MUST CHECK NOW

## Your Error: "Failed to fetch"

This means your environment variables aren't set in Vercel.

---

## ✅ FIX IT NOW (5 minutes)

### Step 1: Go to Vercel Dashboard
👉 https://vercel.com/kochiomris-projects/koch-coaching

### Step 2: Click "Settings" tab

### Step 3: Click "Environment Variables" on left sidebar

### Step 4: Check if these exist:

**CRITICAL - Must have all of these:**

```
NEXT_PUBLIC_SUPABASE_URL
Value: https://adpfjvazlykuravwrhtu.supabase.co

NEXT_PUBLIC_SUPABASE_ANON_KEY
Value: eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImFkcGZqdmF6bHlrdXJhdndyaHR1Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3MjkxODY3NjcsImV4cCI6MjA0NDc2Mjc2N30.GfKpXPK6EQnbtKMTnqhSJ_Xd-EmpxGc0rHIgKXvWDWQ

NEXT_PUBLIC_GOOGLE_CLIENT_ID
Value: 171559689406-nbpiio0jfoj13o183fhtpen045c97ic3.apps.googleusercontent.com

ADMIN_PASSWORD
Value: Omrikoch1212!

GOOGLE_SERVICE_ACCOUNT_EMAIL
Value: koch-calendar@myoauth-project-441523.iam.gserviceaccount.com

GOOGLE_CALENDAR_ID
Value: omrikochman@gmail.com

ADMIN_EMAIL
Value: omrikochman@gmail.com

GOOGLE_PRIVATE_KEY
Value: -----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----
```

### Step 5: For each variable:
- Click "Add New"
- Paste the Name and Value
- Select: ☑️ Production ☑️ Preview ☑️ Development
- Click "Save"

### Step 6: After adding all variables:
- Go back to "Deployments" tab
- Click the 3 dots (...) on latest deployment
- Click "Redeploy"
- Wait 2-3 minutes

---

## ✅ THEN TEST:

1. **Landing page:** https://koch-coaching.vercel.app (should work - no login)
2. **Admin login:** https://koch-coaching.vercel.app/admin/login
   - Password: `Omrikoch1212!`
   - Should work after env vars are set

3. **Create demo account:**
   - Go to: https://koch-coaching.vercel.app/portal/login
   - Click "Register"
   - Fill in:
     - Name: Demo Client
     - Email: demo@koch-coaching.com
     - Password: Demo2024!
   - Click "Sign Up"

---

## 📞 NEED HELP?

**Screenshot what you see in Vercel → Environment Variables**

Send me a screenshot and I'll help you verify they're correct!
